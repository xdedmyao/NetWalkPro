...
def corrupt(x):
    r = tf.add(
        ...
        tf.cast(tf.random.uniform(shape=tf.shape(x), minval=0, maxval=0.1, dtype=tf.float32), tf.float32)
    )
...
def autoencoder(data, corrupt_prob, dimensions, beta=0.01, rho = 0.4, activation = tf.nn.sigmoid, lamb = 0.01, gamma =0.01):
    ...
    init_uniform_unit = tf.compat.v1.keras.initializers.VarianceScaling(
        scale=1.0, mode="fan_avg", distribution="uniform", seed=24
    )
    ...
    for layer_i, n_output in enumerate(dimensions[1:]):
        ...
        W = tf.compat.v1.get_variable(W_name, shape=[n_output, n_input], initializer=init_uniform_unit)
    ...
    kl = tf.reduce_mean(
        rho * tf.math.log(rho / rhohats) + (1 - rho) * tf.math.log((1 - rho) / (1 - rhohats)))
